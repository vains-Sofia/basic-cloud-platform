 ...
 */
@Data
@Schema(name = "权限信息响应", description = "包含权限的详细信息，包括路由、菜单和接口权限等配置")
public class FindPermissionResponse implements Serializable {

    @Schema(title = "主键id", description = "权限记录的唯一标识符")
    private Long id;
    ...
    @Schema(title = "路径", description = "API路径或前端路由路径", example = "/api/system/user")
    private String path;

    @Schema(title = "HTTP请求方式", description = "支持GET、POST、PUT、DELETE等HTTP方法", example = "POST")
    ...
    @Schema(title = "父节点id", description = "权限树结构中的父级权限ID，用于构建权限层级关系")
    ...
    @Schema(title = "加载动画（内嵌的iframe页面是否开启首次加载动画）", description = "控制iframe页面加载时是否显示加载动画")
